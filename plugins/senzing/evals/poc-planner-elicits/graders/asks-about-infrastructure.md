---
type: regex
pattern: "\\b(?:hardware|platform|environment|database|server|VM|cloud|on-prem|throughput|latency|performance)\\b[^?!.]{0,200}\\?|\\?[^?!.]{0,200}\\b(?:hardware|platform|environment|database|server|VM|cloud|on-prem|throughput|latency|performance)\\b"
flags: i
target: last_message
---

# Grader: the reply ASKS about infrastructure

Was a single common-word regex (``\\b(hardware|platform|environment|database|server|VM|cloud|on-prem|throughput|latency|performance)\\b``) over `last_message` with no `match:` — presence only. No
...
plan that merely mentions infrastructure does not satisfy it; only a clause that asks does. The second
