---
type: regex
pattern: "\\b(?:data|sources?|records?|datasets?|files?)\\b[^?!.]{0,200}\\?|\\?[^?!.]{0,200}\\b(?:data|sources?|records?|datasets?|files?)\\b"
flags: i
target: last_message
---

# Grader: the reply ASKS about the data

Was a single common-word regex (``\\b(data|source|record)s?\\b``) over `last_message` with no `match:` — presence only. No
...
plan that merely mentions the data does not satisfy it; only a clause that asks does. The second
